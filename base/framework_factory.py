...
def build_network(net_name, config):
    ...
    if not any(config['backbone'] in name for name in all_names):
        print(f"Backbone \"{config['backbone']}\" is not available now. Please use one of the backbones in {all_names}.")
        sys.exit()
    else:
