...
def maefm(preds, target, config, alpha=0.5, beta=0.5):
    ...
    EPS = 1e-8
    N = pred.size(0)
    th = 2 * pred.mean(dim=(2,3), keepdim=True)
    th = th.clamp(max=1.0)
    binary_pred = (pred >= th).float()
    binary_gt = (target > 0.5).float()

    tp = (binary_pred * binary_gt).view(N, -1).sum(dim=1)
    precision = tp / (binary_pred.view(N, -1).sum(dim=1) + EPS)
    recall = tp / (binary_gt.view(N, -1).sum(dim=1) + EPS)
    fm = 1.3 * precision * recall / (0.3 * precision + recall + EPS)
    fm_loss = 1 - fm.mean()

    total_loss = alpha * mae_loss + beta * fm_loss
    return total_loss
...
def loss_to_dict(loss_config):
    ...
    for term in loss_terms:
        ...
        # 注册自定义loss
        if loss_name == 'maefm':
            loss_cluster[ltype].append([maefm, float(weight)])
        else:
